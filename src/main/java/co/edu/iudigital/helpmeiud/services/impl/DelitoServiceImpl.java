...
public class DelitoServiceImpl implements IDelitoService {
    ...
    @Override
    public List<Delito> obtenerDelito() {
        List<Delito> delitos = delitoRepository.findAll();
        return delitos;
    }

    @Override
    public Delito obtenerDelitoPorID(Long id) {
        Delito delito = delitoRepository.findById(id).orElseThrow( () -> new RuntimeException("No existe delito"));
        return delito;
    }
